...
public class SPIEL
{
    ...
    private boolean Schachmatt;
    private String Sieger;
    private boolean Schach;
    private String F_K;
    private boolean KbewInSch; //Koenig waere nach dem bewegen im Schach
    private boolean RochadeNichtM;

    private BILD[] hintergrund;

    //Figuren
    private FIGUR[] BauerW;
    private FIGUR[] LaeuferW;
    private FIGUR[] TurmW;
    private FIGUR[] SpringerW;
    private FIGUR   DameW;
    private FIGUR   KoenigW;

    private FIGUR[] BauerS;
    private FIGUR[] LaeuferS;
    private FIGUR[] TurmS;
    private FIGUR[] SpringerS;
    private FIGUR   DameS;
    private FIGUR   KoenigS;

    private FIGUR[][] SF; //Spielfeld

    private int FeldAusgew;
    private boolean FigurAusgew;
    ...
    public boolean FigurBewegen(int von, int zu)
    {
        ...
        if(a.Art.equals("Turm")){bewegungMoeglich = BTurm(u_a, r_a, u_z, r_z, "Bew");}
        if(a.Art.equals("Springer")){bewegungMoeglich = BSpringer(u_a, r_a, u_z, r_z, "Bew");}
        if(a.Art.equals("Laeufer")){bewegungMoeglich = BLaeufer(u_a, r_a, u_z, r_z, "Bew");}
        if(a.Art.equals("Koenig")){bewegungMoeglich = BKoenig(u_a, r_a, u_z, r_z, "Bew");}
        if(a.Art.equals("Dame")){bewegungMoeglich = BDame(u_a, r_a, u_z, r_z, "Bew");}
    ...
    public boolean BTurm(int u_a, int r_a, int u_z, int r_z, String Aufrufer)
    {
        if(u_a != u_z && r_a == r_z)
        {
            int i = u_a + 1;
            ...
            while(i < a)
            {
                if(SF[i][r_z] != null)
                {
                    if(SF[i][r_z].Art.equals("Koenig") && Aufrufer.equals("test"))
                    {
                        return true;
                    }
                    else
        ...
        if(u_a == u_z && r_a != r_z)
        {
            ...
            for(; i < a; i++)
            {
                if(SF[u_z][i] != null)
                {
                    if(SF[u_z][i].Art.equals("Koenig") && Aufrufer.equals("test"))
                    {
                        return true;
                    }
                    else
    ...
    public boolean BSpringer(int u_a, int r_a, int u_z, int r_z, String Aufrufer)
    {
    ...
    public boolean BLaeufer(int u_a, int r_a, int u_z, int r_z, String Aufrufer)
    {
        int y = u_z - u_a;
        int x = r_z - r_a;
        ...
        for(; i < a; i++)
        {
            if(SF[u_a+i*vy][r_a+i*vx] != null)
            {
                if(SF[u_a+i*vy][r_a+i*vx].Art.equals("Koenig") && Aufrufer.equals("test"))
                {
                    return true;
                }
                else
    ...
    public boolean BDame(int u_a, int r_a, int u_z, int r_z, String Aufrufer)
    {
        if(BTurm(u_a, r_a, u_z, r_z, Aufrufer) || BLaeufer(u_a, r_a, u_z, r_z, Aufrufer))
        {
            return true;
        }
        ...
        for(int u = 0; u<8; u++)
        {
            for(int r = 0; r<8; r++)
            {
                ...
                if(SF[u][r] != null && !SF[u][r].Art.equals("Koenig"))
                {
                    ...
                    if(a.Art.equals("Turm")){bewegungMoeglich = BTurm(u_a, r_a, u_z, r_z, "test");}
                    if(a.Art.equals("Springer")){bewegungMoeglich = BSpringer(u_a, r_a, u_z, r_z, "test");}
                    if(a.Art.equals("Laeufer")){bewegungMoeglich = BLaeufer(u_a, r_a, u_z, r_z, "test");}
                    if(a.Art.equals("Dame")){bewegungMoeglich = BDame(u_a, r_a, u_z, r_z, "test");}
