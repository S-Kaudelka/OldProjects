...
public class WELLEN
{
    ...
    //Spawnpunkt
    public Point Spawnpunkt;
    public int erzX; // -1 links; 0 oben/unten; 1 rechts
    public int erzY; // -1 oben; 0 links/rechts; 1 unten
    ...
    public void GE(int Von, int Bis, int Typ)
    {
        ...
        {
            s.Gegner[i] = new GEGNER(Typ, new WEG(s.SpielLevel));
            x = Spawnpunkt.x+(i*55*erzX);
            y = Spawnpunkt.y+(i*55*erzY);
            s.Gegner[i].Skin.SetzeX(x);
            s.Gegner[i].Skin.SetzeY(y);
    ...
    public void WelleSchicken(int W)
    {
        ...
        // Wenn alle wellen vorbei sind
        s.c.start.Running = true;
        if(s.c.start.StageMax < s.SpielLevel)
        {
            s.c.start.StageMax = s.SpielLevel;
        }
        START st = s.c.start;
        st.Anzeigen(true);
        st.Speichern();
